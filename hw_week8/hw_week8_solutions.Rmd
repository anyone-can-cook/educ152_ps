...
The purpose of this short homework assignment is to get some practice with the basics of multivariate regression. You will:
...
- The categorical independent variable is "school control" `bysctrl`
...
- OLS prediction line without estimates
  - $\hat{Y_i} = \hat{\beta_0} + \hat{\beta_1}X_i + \hat{\beta_2} X_{2i}+ \hat{\beta_3} X_{3i}$
- OLS prediction line with estimates
  - $\hat{Y_i} =$  `r format(round(summary(mod1)$coefficients[1,1], digits =2),big.mark = ',')` + `r format(round(summary(mod1)$coefficients[2,1], digits =2),big.mark = ',')` $\times X_{1i}$ + `r format(round(summary(mod1)$coefficients[3,1], digits =2),big.mark = ',')` $\times X_{2i}$ + `r format(round(summary(mod1)$coefficients[4,1], digits =2),big.mark = ',')` $\times X_{3i}$

#### 3. Interpret the value of regression coefficients $\hat{\beta_1}$, $\hat{\beta_2}$, and $\hat{\beta_3}$ in words

__YOUR ANSWER HERE__: 

- $X_{1i}$ (parental income in \$ thousands); $\hat{\beta_1}=$ `r format(round(summary(mod1)$coefficients[2,1], digits =2),big.mark = ',')`
  - a one thousand dollar increase in parental income associated with a `r format(round(summary(mod1)$coefficients[2,1], digits =2),big.mark = ',')` point change in high school reading test score
- $X_{2i}$ (attends a Catholic private school); $\hat{\beta_2}=$ `r format(round(summary(mod1)$coefficients[3,1], digits =2),big.mark = ',')`
  - attending a Catholic private school as opposed to a public school is, on average, associated with a `r format(round(summary(mod1)$coefficients[3,1], digits =2),big.mark = ',')` change in high school reading test score, holding the of value all other $X$ variables constant
- $X_{3i}$ (attends a non-Catholic private school); $\hat{\beta_3}=$ `r format(round(summary(mod1)$coefficients[4,1], digits =2),big.mark = ',')`
  - attending a non-Catholic private school as opposed to a public school is, on average, associated with a `r format(round(summary(mod1)$coefficients[4,1], digits =2),big.mark = ',')` change in high school reading test score, holding the of value all other $X$ variables constant  

#### 4. Interpret the value of regression coefficients $\hat{\beta_0}$ in words

__YOUR ANSWER HERE__: 

- $\hat{\beta_0}=$ `r format(round(summary(mod1)$coefficients[1,1], digits =2),big.mark = ',')`
- The estimated average high school test score for a student with parental income of `0` who attended a public high school is `r format(round(summary(mod1)$coefficients[1,1], digits =2),big.mark = ',')`
...
- $\hat{Y_i} =$ `r format(round(summary(mod1)$coefficients[1,1], digits =2),big.mark = ',')` + `r format(round(summary(mod1)$coefficients[2,1], digits =2),big.mark = ',')` $\times 150$ + `r format(round(summary(mod1)$coefficients[4,1], digits =2),big.mark = ',')` $\times 1=$
