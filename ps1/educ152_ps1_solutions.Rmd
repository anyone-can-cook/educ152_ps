...
plot_t_distribution <- function(data_vec, mu, alpha = 0.05, alternative = 'two.sided', plot_title = '', shade_rejection = TRUE, shade_pval = FALSE) {
  ...
  p <- p +
    annotate('text', size = 9*5/14, x = 4.81, y = 0.14, hjust = 0,
             label = 'bold(Statistics)', parse = T) +
    annotate('text', size = 8*5/14, x = 4.86, y = 0:4 * -0.015 + 0.12, hjust = 0,
             label = legend_text)
  ...
  p <- p +
    ...
    scale_color_manual(name = 'Legend',
                       breaks = c('tstat', 'cval'),
                       labels = c('t-statistics / p-value', 'critical value / alpha'),
                       values = c(tstat = 'blue', cval = 'red')) +
    theme(plot.title = element_text(size = 10, face = 'bold', hjust = 0.5),
          plot.margin = unit(c(5.5, 28, 5.5, 5.5), 'pt'),
          legend.title = element_text(size = 9, face = 'bold'),
          legend.text = element_text(size = 8)) +
...
- You can either:
  - Share something you learned from this problem set or the course. Make sure to mention the instructors (@ozanj, @Patricia Martín).
