...
#### X. In social science research a "mechanism" is an explanation for why one variable ($X$) has a causal effect on another variable ($Y$). In your own words, what are 2-3 mechanisms for why participation in the MAS program would have a positive causal effect on the probability of graduating from high school
...
#### X. In observational data (unit x not randomly assigned to values of $X$), when researchers use regression to estimate the causal effect of $X$ on $Y$, explain the primary strategy researchers use to eliminate (or at least reduce) omitted variable bias?

- __YOUR ANSWER HERE__:  text of answer

#### X. The "linear probability model" basically means applying ordinary least squares (OLS) regression to a dichotomous (0/1) outcome variable as opposed to a continuous outcome variable. In the linear probability model, what is the "generic" interpretation of $\hat{\beta}_1$ when $X_1$ is a categorical variable?
...
- __YOUR ANSWER HERE__:  text of answer
  ...
  - taking two MAS courses as opposed to taking zero MAS courses ....
...
- __YOUR ANSWER HERE__: Developmental math courses ($X$) may have a negative causal effect on the number of postsecondary credits earned, ($Y$), because having to take developmental math courses, that perhaps do not count towards postsecondary credit, may extend a students' time to degree depending on how many developmental math courses they are required to take and/or having to pass these courses in order to enroll in courses that count towards postsecondary credit. 
...
- Interpretation of $\hat{\beta_1}=$ `r format(round(summary(cred_mod1)$coefficients[2,1], digits =2),big.mark = ',')` (1 developmental math course)
  - "Taking one developmental math course as opposed to zero developmental math courses is, on average, associated with a `r format(round(summary(cred_mod1)$coefficients[2,1], digits =2),big.mark = ',')` credit increase in postsecondary credits earned."
- Interpretation of $\hat{\beta_2}=$ `r format(round(summary(cred_mod1)$coefficients[3,1], digits =2),big.mark = ',')` (2+ more developmental math courses)
  - "Taking two or more developmental math courses as opposed to zero developmental math courses is, on average, associated with a `r format(round(summary(cred_mod1)$coefficients[3,1], digits =2),big.mark = ',')` credit increase in postsecondary credits earned." 
...
- __YOUR ANSWER HERE__:  The $R^2$ is the fraction of the variance in Y that is explained by X (and is not already explained by $\bar{Y}$). 
...
- __YOUR ANSWER HERE__: Sample standard error of the regression (SER) is an estimate of how far away, on average, the observed value of $Y_i$ is from the predicted value $\hat{Yi}$ of $Y_i$ for a random observation $i$. 
...
- __YOUR ANSWER HERE__: Sample standard deviation of a variable is an estimate of how far away, on average, the observed value of $Y_i$ is from the sample mean $\bar{Yi}$ of $Y_i$ for a random observation $i$. 
...
#### 6. Now, we will start adding "control" variables to our model. We will add ,`bytxmstd`, a continuous measure of high school standardized math score, and `f2everdo` a dichotomous categorical measure of whether the student ever had a "dropout episode" in high school. Run the below code which provides descriptive staitistics of these variables. Separately for each of these variables, answer the following: Why might this variable satisfy the fist condition of omitted variable bias? Why might this variable satisfy the second condition of omitted variable bias?

- __YOUR ANSWER HERE__:  

- The variable `bytxmstd`
    1. High school standardize math score, `bytxmstd`, affects value of postsecondary credits earned (e.g. scoring high on standardized math score could suggest students are prepared for college-level courses and may enroll in more courses).
    2. High school standardize math score, `bytxmstd`, has a relationship with developmental courses taken (e.g., score high on standardized math score means less, if at all, developmental courses to take).
    
- The variable `f2everdo`
    1. Dropout episode in high school, `f2everdo`, affects value of postsecondary credits earned (e.g. perhaps student may take a break from college, for many reasons, and take less courses (earn less postsecondary credits)).
    2. Dropout episode in high school, `f2everdo`, has a relationship with developmental courses taken (e.g., if a student dropped out at some point in high school, they may have missed important math related courses and will perhaps need to take more developmental math courses to prepare for college-level math courses).
...
- 1. The variable `f1pared`, could affect our outcome, postsecondary credits earned, because students who have a parent with a higher level of education may have more postsecondary credits earned (e.g., say for example if they took community college courses in high school). 
...
- 1. The variable `parent_income` could affect our outcome, postsecondary credits earned because perhaps students who have a higher parental income could earn more postsecondary credits if they took community college courses in high school and/or took AP courses in high school to earn postsecondary credit. 
...
Run the below model of the relationship between developmental math ($X$) and college credits earned ($Y$) with no control variables and then do the following
...
- Write a paragraph or two to explain your reasoning.
