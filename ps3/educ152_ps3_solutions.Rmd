...
ADD A SENTENCE OR TWO ABOUT THE MODEL THEY WILL BE RUNNING; WHAT IS THE X VARIABLE; WHAT IS THE Y VAR

I'D LIKE YOU TO CHANGE OUT THE X VARIABLE BECAUSE WE DID Y=EARNINGS AND X = URBAN FOR THE LECTURE; HOW ABOUT DO X=CARNEGIE CLASSIFICATION

```{r}
df_edu_fac %>% count(ccbasic)
```
...
PATRICIA - ADD A QUESTION SOMETHING ALONG THE LINES OF: IN REGRESSION OF RELATIONSHIP BETWEEN CARNEGIE (X) AND EARNINGS (Y), WHICH CATEGORY OF CARNEGIE WILL BE THE REFERENCE GROUP? WHY WILL THIS CATEGORY BE THE REFERENCE GROUP? CAN PROVIDE THEM SOME RELEVANT CODE/OUTPUT FROM R TO GIVE THEM HINT

#### 3.  For your analysis of the relationship between the level of urbanization by institution (X) and institution-level student earnings (Y) for graduates of MA programs in Education, do the following: write out the population linear regression model (label the symbols and write out what the variables $X$ and $Y$ actually represent); write out the OLS prediction line (without estimate values); write out the OLS prediction line (with estimate values); interpet the point estimate value of $\hat{\beta}_0$ in words; interpet the point estimate value(s) of $\hat{\beta}_1$, $\hat{\beta}_2$, $\hat{\beta}_3$ in words.

PATRICIA - MAKE THIS A SEPARATE QUESTION; interpet the point estimate value(s) of $\hat{\beta}_1$, $\hat{\beta}_2$, $\hat{\beta}_3$ in words.

AND MAKE THIS A SEPARATE QUESTION: interpet the point estimate value of $\hat{\beta}_0$ in words;
...
- where:
  ...
  - $X_i$ = level of urbanization (measured by `urban`), which has the following four categories: large city (250k+) [refrence group]; med/small city; suburb; town/rural  
  ...
  - $\beta_3$ = population regression coefficient associated with being a “town/rural” rather than reference group
...
- where:
  ...
  - $\hat{\beta_1}$= predicted value of $Y$ when $X_1$ = 1 “med/small city”.  
  - $\hat{\beta_2}$= predicted value of $Y$ when $X_2$ = 1 “suburb”.    
  - $\hat{\beta_3}$= predicted value of $Y$ when $X_3$ = 1 “town/rural”.
...
- Interpret point estimate value of $\hat{\beta}_0$ & $\hat{\beta}_1$:
  - interpretation of $\hat{\beta_0}=$ 53,853 is the predicted institution-level student earnings for MA graduates of education programs who attended a university located in a large city.
  - Interpretation of $\hat{\beta_1}=$ `r format(round(summary(mod3)$coefficients[2,1], digits =2),big.mark = ',')` (med/small city)
  - "being located in a medium/small city as opposed to large city is, on average, associated with a `r format(round(summary(mod3)$coefficients[2,1], digits =2),big.mark = ',')` dollar change in institution-level student earnings for MA graduates of education programs"
- Interpretation of $\hat{\beta_2}=$ `r format(round(summary(mod3)$coefficients[3,1], digits =2),big.mark = ',')` (suburb)
  - "being located in a suburb as opposed to large city is, on average, associated with a `r format(round(summary(mod3)$coefficients[3,1], digits =2),big.mark = ',')` dollar change in institution-level student earnings for for MA graduates of education programs"
- Interpretation of $\hat{\beta_3}=$ `r format(round(summary(mod3)$coefficients[4,1], digits =2),big.mark = ',')` (town/rural)
  - "being located in a town/rural area as opposed to large city is, on average, associated with a `r format(round(summary(mod3)$coefficients[4,1], digits =2),big.mark = ',')` dollar change in institution-level student earnings for MA graduates of education programs.
  

<span style="color:red"><b>/X<b></span>   

#### 4. Use the OLS prediction line to predict the value of $Y$ for each value of $X$ [show work].    

MODIFY QUESTION ALONG THESE LINES. WHAT IS THE PREDICTED VALUE OF Y FOR EACH OF THE FOLLOWING UNIVERSITY TYPES. SHOW WORK (OLS PREDICTION LINE W/ ESTIMATES; THEN RESULT OF CALCULATION); GIVE STUDENTS THE ANSWER FOR RESEARCH2

- RESEARCH 2: OLS LINE WITHOUT ESTIMATES; OLS LINE WITH ESTIMATES; CALCULATION
- MASTER'S 1
- MASTER'S 2
- RESEARCH 1

- **Hint**: If all other values of the independent variable are 0 ($X_1$, $X_2$,$X_3$), the OLS prediction line would look something like this: $\hat{Y_i} =$  `r format(round(summary(mod3)$coefficients[1,1], digits =2),big.mark = ',')` + `r format(round(summary(mod3)$coefficients[2,1], digits =2),big.mark = ',')` $\times 0$ + `r format(round(summary(mod3)$coefficients[3,1], digits =2),big.mark = ',')` $\times 0$ + `r format(round(summary(mod3)$coefficients[4,1], digits =2),big.mark = ',')` $\times 0$  
- And we would predict the value of $Y$ for the reference group to look like: $\hat{Y_i} =$  `r format(round(summary(mod3)$coefficients[1,1], digits =2),big.mark = ',')`   

Now do the following for each value of X  

  - Reference group (given above) 
  - Non reference group 1 ($X_1$) =  (med/small city) 
  - Non reference group 2 ($X_2$) =  (suburb)
  - Non reference group 3 ($X_3$) =  (town/rural)
  
- __YOUR ANSWER HERE__:  

Non reference group 1 ($X_1$) = $\hat{Y_i} =$  `r format(round(summary(mod3)$coefficients[1,1], digits =2),big.mark = ',')` + `r format(round(summary(mod3)$coefficients[2,1], digits =2),big.mark = ',')` $\times X_{1i}$ + `r format(round(summary(mod3)$coefficients[3,1], digits =2),big.mark = ',')` $\times 0$ + `r format(round(summary(mod3)$coefficients[4,1], digits =2),big.mark = ',')` $\times 0$   
...
#### 5. Write out the null and alternative hypotheses for each non-reference group category of variable $X$.   
...
#### 6. Write out the formula for the t-statistic of $\hat{\beta}_k$. 
...
#### 7. For one of the non-reference group categories of $X$ do the following:  
